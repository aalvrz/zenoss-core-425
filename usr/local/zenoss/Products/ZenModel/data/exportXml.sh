...
if [ "$dbtype" = "mysql" ]; then
    ...
    if [ "$innodb_format" != "barracuda" ]; then
        echo "Error, your mysql innodb file format is not (at least) Barracuda. Current file format is $innodb_format"
        echo "  To resolve: set 'innodb_file_format = Barracuda' in /etc/mysql/my.cnf and restart mysql"
        exit 1
    fi
    ...
    if [ "$object_state_format" != "compressed" ]; then
        echo "Error object_state table is not compressed. Current object state format is $object_state_format"
        echo "  To resolve: set 'innodb_file_per_table = 1' in /etc/mysql/my.cnf and rebuild zodb database"
        echo "  To rebuild zodb:"
        echo "    mysqldump -u root zodb > zodb.sql"
        echo "    mysql -u root"
        echo "      use zodb ; source zodb.sql;"
        exit 1
    fi
fi
