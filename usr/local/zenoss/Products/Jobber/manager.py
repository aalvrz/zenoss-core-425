...
class JobManager(ZenModelRM):
    ...
    def addJob(self, klass, description=None, args=None, kwargs=None, properties=None):
        ...
        try:
            description = description or job.getJobDescription(*args, **kwargs)
        except Exception:
        ...
        for prop, propval in properties.iteritems():
            setattr(jobrecord, prop, propval)

        self._setOb(job_id, jobrecord)
        jobrecord = self._getOb(job_id)
        self.getCatalog().catalog_object(jobrecord)
        log.info("Created job %s: %s", klass, job_id)
    ...
    def getJob(self, jobid):
        ...
        try:
            ...
            raise NoSuchJobException('Job not present: %s: %s' % (jobid, e)), None, sys.exc_info()[2]
